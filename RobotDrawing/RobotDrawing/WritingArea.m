...
//NSLog(@"%@", _path);
    ...
    //NSLog(@"%@", _path);
//    NSLog(@"%@", [self.arrPath lastObject]);
    
    NSValue *pathValue = [NSValue valueWithPointer:_path];
    [_arrPath addObject:pathValue];
    
    NSLog(@"array : %@", _arrPath);
    
    //NSData *jsonData = [NSJSONSerialization dataWithJSONObject:self.arrPath options:NSJSONWritingPrettyPrinted error:&error];
    //NSString *jsonString = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
    
    //vc.drawingData = jsonData;
}
